...
public class VehicleInformationService : IVehicleInformationService
{
    ...
    public VehicleInformationService(IRDWService rdwService)
    {
    ...
    private List<List<string>> GetCardInfo(JToken data, JToken fuelInfo)
    {
        var from = data.GetSafeDateYearValue("datum_eerste_toelating_dt");
        var brandDescription = $"{data.GetSafeValue("merk")} ({data.GetSafeValue("handelsbenaming")}) uit {from}";

        var amount = fuelInfo.GetSafeDecimalValue("brandstofverbruik_gecombineerd");
        var usagesDecription = amount != 0
            ? $"{100M / amount:F2}KM op 1 liter {fuelInfo.GetSafeValue("brandstof_omschrijving").ToLower()}"
            : "Niet geregistreerd";

        return new List<List<string>> {
            new() { "Merk", brandDescription },
            new() { "Verbruik", usagesDecription },
